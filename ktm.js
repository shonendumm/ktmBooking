...
const dateReturnArray = ['21/09/2019','22/09/2019','28/09/2019']; // list of dates coming back. Same since day trip
...
async function start() {
    ...
    async function goKTM(date) {
        ...
        await page.waitFor(12000); // adjust wait time longer if TypeError: Cannot read property 'innerText' of null
...
async function back() {
    ...
    async function returnKTM(date) {
        ...
        await page.click('#taboneway > form > ul:nth-child(3) > li:nth-child(3) > input'); // click submit
    ...
    for (var i = 0; i < dateReturnArray.length; i++) { // iterate through dates provided
        let trainRes = await returnKTM(dateReturnArray[i]);
        let timing;
        switch (timeReturn) {
            case 1:
                timing = 'all timings';
        ...
        if (trainRes.includes('sorry')) {
            console.log(`No return train for ${dateGoArray[i]} at ${timing}`);
        } else { // else if x.includes(positive results)
        console.log(`Success! Got return train for ${dateGoArray[i]} at ${timing}, go book it`);
        repeat = false; 
        };
    };
